...
const MyProducts = () => {
  ...
  const resetForm = () => {
    ...
    setFormOriginalPrice(''); setFormStock(''); setFormCategory(categories[0]?.name || '');
    setFormError(''); setEditingProduct(null);
  };
  ...
  useEffect(() => {
    ...
    if (activeRows.length > 0) {
      gsap.fromTo(activeRows, { opacity: 0, x: -10 }, { opacity: 1, x: 0, duration: 0.4, stagger: 0.03, ease: 'power2.out', clearProps: 'all' });
    }
  }, [search, filterCategory, filtered.length]);

  useEffect(() => {
    if (drawerOpen && overlayRef.current && drawerRef.current) {
  ...
  const closeDrawer = () => {
    if (overlayRef.current && drawerRef.current) {
      ...
      gsap.to(drawerRef.current, { x: '100%', duration: 0.4, ease: 'power3.in', onComplete: () => { setDrawerOpen(false); resetForm(); } });
    } else { setDrawerOpen(false); resetForm(); }
  };

  const statusVariant = (status: string) => status === 'active' ? 'success' : status === 'draft' ? 'warning' : 'danger';
  ...
  return (
    <div ref={containerRef} className="opacity-0">
      ...
      {filtered.length > 0 ? (
        ...
        <div className="bg-[#050505] border border-white/[0.08] rounded-2xl p-16 text-center">
          <div className="h-16 w-16 rounded-full bg-white/[0.02] border border-white/[0.05] flex items-center justify-center mx-auto mb-4">
            <Package className="h-7 w-7 text-white/15" />
          </div>
          <p className="text-base text-white/60 font-medium mb-2">{search ? 'No products match your search' : 'No products yet'}</p>
          <p className="text-sm text-white/30 mb-6">{search ? 'Try a different search term.' : 'Click "Add Product" to list your first item for sale.'}</p>
          {!search && <Button onClick={() => openDrawer()} leftIcon={<Plus className="h-4 w-4" />}>Add Product</Button>}
        </div>
      )}

      {/* Slide-in Drawer */}
      <div className={`fixed inset-0 z-40 ${drawerOpen ? 'pointer-events-auto' : 'pointer-events-none hidden'}`}>
        <div ref={overlayRef} className="absolute inset-0 bg-black/80 backdrop-blur-sm opacity-0" onClick={closeDrawer} />
        <div ref={drawerRef} className="absolute right-0 top-0 h-full w-full max-w-md bg-[#050505] border-l border-white/[0.08] shadow-[-20px_0_50px_rgba(0,0,0,0.5)] z-50 overflow-y-auto translate-x-full">
          <div className="p-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-xl font-bold text-white tracking-tight">{editingProduct ? 'Edit Product' : 'Add New Product'}</h2>
              <button onClick={closeDrawer} className="p-2 rounded-xl text-white/40 hover:text-white hover:bg-white/10 transition-all"><X className="h-5 w-5" /></button>
            </div>

            {formError && <div className="mb-4 px-4 py-3 bg-danger/10 border border-danger/20 rounded-xl text-sm text-danger">{formError}</div>}
            ...
            <div className="space-y-6">
              ...
              <Input label="Product Name" placeholder="Enter product name" value={formName} onChange={(e) => setFormName(e.target.value)} />
              <div>
                <label className="block text-sm font-bold text-white mb-2">Description</label>
                <textarea placeholder="Describe your product..." rows={4} value={formDescription} onChange={(e) => setFormDescription(e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input label="Price" type="number" placeholder="0.00" value={formPrice} onChange={(e) => setFormPrice(e.target.value)} />
                <Input label="Original Price" type="number" placeholder="Optional" value={formOriginalPrice} onChange={(e) => setFormOriginalPrice(e.target.value)} />
              </div>
              <Input label="Stock" type="number" placeholder="0" value={formStock} onChange={(e) => setFormStock(e.target.value)} />
              <div>
